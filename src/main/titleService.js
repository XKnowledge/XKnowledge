...
/**
 * 未保存圆点的双位置标题：display 供应用内自绘标题条（圆点在展示名后，
 * 对齐 VSCode 标签页），taskbar 供任务栏/Alt-Tab（圆点在整标题前）。
 * dirty 为假值时两值相同。
 */
 ...
 * 循环必然终止。entry 可带 dirty: boolean（缺省视为干净）。
 * 返回 Map<webContentsId, { display, taskbar }>。
 */
export const computeTitles = (entries) => {
  ...
  for (const group of groups.values()) {
    if (group.length === 1) {
      titles.set(
        group[0].webContentsId,
        composeWindowTitles(displayOf(group[0].path, 0), group[0].dirty)
      )
      continue
    }
    ...
    for (const m of group) {
      titles.set(m.webContentsId, composeWindowTitles(displayOf(m.path, m.k), m.dirty))
    }
  }
  return titles
}
